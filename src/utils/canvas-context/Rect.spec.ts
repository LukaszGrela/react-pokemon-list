...
describe('utils', () => {
  describe('canvas-context', () => {
    describe('Rect', () => {
      ...
      type TTestCase = [name: string, t: number, r: number, b: number, l: number, x: number, y: number, w: number, h: number, empty: boolean];
      test.each<TTestCase>([
      ...
      ])('Creates valid rect with correct size for TRBL %s', (trbl,
        t, r, b, l,
        x, y, w, h,
        empty) => {
      ...
      it('returns correct toString', () => {
        expect(new Rect(0, 0, 0, 0).toString()).toEqual("Rect: trbl=0,0,0,0|xywh=0,0,0,0")
        expect(new Rect(1, 1, 1, 1).toString()).toEqual("Rect: trbl=1,1,1,1|xywh=1,1,0,0")
        expect(new Rect(1, 2, 2, 1).toString()).toEqual("Rect: trbl=1,2,2,1|xywh=1,1,1,1")
        expect(new Rect(-1, -1, -1, -1).toString()).toEqual("Rect: trbl=-1,-1,-1,-1|xywh=-1,-1,0,0")
        expect(new Rect(-1, 0, 0, -1).toString()).toEqual("Rect: trbl=-1,0,0,-1|xywh=-1,-1,1,1")
      })
